// Code generated by entc, DO NOT EDIT.

package ent

import (
	"fmt"
	"strings"

	"github.com/facebookincubator/ent/dialect/sql"
	"github.com/team07/app/ent/insurance"
)

// Insurance is the model entity for the Insurance schema.
type Insurance struct {
	config `json:"-"`
	// ID of the ent.
	ID int `json:"id,omitempty"`
	// Classofinsurance holds the value of the "classofinsurance" field.
	Classofinsurance string `json:"classofinsurance,omitempty"`
	// Company holds the value of the "company" field.
	Company string `json:"company,omitempty"`
	// Edges holds the relations/edges for other nodes in the graph.
	// The values are being populated by the InsuranceQuery when eager-loading is set.
	Edges InsuranceEdges `json:"edges"`
}

// InsuranceEdges holds the relations/edges for other nodes in the graph.
type InsuranceEdges struct {
	// Insuranceof holds the value of the insuranceof edge.
	Insuranceof []*Ambulance
	// loadedTypes holds the information for reporting if a
	// type was loaded (or requested) in eager-loading or not.
	loadedTypes [1]bool
}

// InsuranceofOrErr returns the Insuranceof value or an error if the edge
// was not loaded in eager-loading.
func (e InsuranceEdges) InsuranceofOrErr() ([]*Ambulance, error) {
	if e.loadedTypes[0] {
		return e.Insuranceof, nil
	}
	return nil, &NotLoadedError{edge: "insuranceof"}
}

// scanValues returns the types for scanning values from sql.Rows.
func (*Insurance) scanValues() []interface{} {
	return []interface{}{
		&sql.NullInt64{},  // id
		&sql.NullString{}, // classofinsurance
		&sql.NullString{}, // company
	}
}

// assignValues assigns the values that were returned from sql.Rows (after scanning)
// to the Insurance fields.
func (i *Insurance) assignValues(values ...interface{}) error {
	if m, n := len(values), len(insurance.Columns); m < n {
		return fmt.Errorf("mismatch number of scan values: %d != %d", m, n)
	}
	value, ok := values[0].(*sql.NullInt64)
	if !ok {
		return fmt.Errorf("unexpected type %T for field id", value)
	}
	i.ID = int(value.Int64)
	values = values[1:]
	if value, ok := values[0].(*sql.NullString); !ok {
		return fmt.Errorf("unexpected type %T for field classofinsurance", values[0])
	} else if value.Valid {
		i.Classofinsurance = value.String
	}
	if value, ok := values[1].(*sql.NullString); !ok {
		return fmt.Errorf("unexpected type %T for field company", values[1])
	} else if value.Valid {
		i.Company = value.String
	}
	return nil
}

// QueryInsuranceof queries the insuranceof edge of the Insurance.
func (i *Insurance) QueryInsuranceof() *AmbulanceQuery {
	return (&InsuranceClient{config: i.config}).QueryInsuranceof(i)
}

// Update returns a builder for updating this Insurance.
// Note that, you need to call Insurance.Unwrap() before calling this method, if this Insurance
// was returned from a transaction, and the transaction was committed or rolled back.
func (i *Insurance) Update() *InsuranceUpdateOne {
	return (&InsuranceClient{config: i.config}).UpdateOne(i)
}

// Unwrap unwraps the entity that was returned from a transaction after it was closed,
// so that all next queries will be executed through the driver which created the transaction.
func (i *Insurance) Unwrap() *Insurance {
	tx, ok := i.config.driver.(*txDriver)
	if !ok {
		panic("ent: Insurance is not a transactional entity")
	}
	i.config.driver = tx.drv
	return i
}

// String implements the fmt.Stringer.
func (i *Insurance) String() string {
	var builder strings.Builder
	builder.WriteString("Insurance(")
	builder.WriteString(fmt.Sprintf("id=%v", i.ID))
	builder.WriteString(", classofinsurance=")
	builder.WriteString(i.Classofinsurance)
	builder.WriteString(", company=")
	builder.WriteString(i.Company)
	builder.WriteByte(')')
	return builder.String()
}

// Insurances is a parsable slice of Insurance.
type Insurances []*Insurance

func (i Insurances) config(cfg config) {
	for _i := range i {
		i[_i].config = cfg
	}
}
