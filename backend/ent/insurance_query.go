...
type InsuranceQuery struct {
	...
	// eager-loading edges.
	withInsuranceof *AmbulanceQuery
...
// QueryInsuranceof chains the current query on the insuranceof edge.
func (iq *InsuranceQuery) QueryInsuranceof() *AmbulanceQuery {
	query := &AmbulanceQuery{config: iq.config}
...
//  WithInsuranceof tells the query-builder to eager-loads the nodes that are connected to
// the "insuranceof" edge. The optional arguments used to configure the query builder of the edge.
func (iq *InsuranceQuery) WithInsuranceof(opts ...func(*AmbulanceQuery)) *InsuranceQuery {
	query := &AmbulanceQuery{config: iq.config}
	for _, opt := range opts {
		opt(query)
	}
	iq.withInsuranceof = query
	return iq
}
...
func (iq *InsuranceQuery) sqlAll(ctx context.Context) ([]*Insurance, error) {
	...
	if query := iq.withInsuranceof; query != nil {
		...
		query.Where(predicate.Ambulance(func(s *sql.Selector) {
			s.Where(sql.InValues(insurance.InsuranceofColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			...
			node.Edges.Insuranceof = append(node.Edges.Insuranceof, n)
		}
	}
